...
> Monkeys Server 是 Monkeys 的核心服务，对外暴露了 Node.js 应用的 CPU、内存、网络等指标。
...
- `global.scrape_interval`：监控间隔时间，推荐为 `5s`。
- `scrape_configs[].metrics_path`： Monkeys Server 服务的 Prometheus 指标接口为 `/metrics`。
- `scrape_configs[].static_configs[].targets`：Monkeys Server 服务的地址和端口，你可能需要根据部署的实际情况进行修改。
...
- 选择**导入**选项。
- 在 *Grafana.com 仪表盘网址或 ID* 中填入 `11159` (https://grafana.com/grafana/dashboards/11159-nodejs-application-dashboard/)。
...
> Monkeys 服务本身不会部署 vLLM，如果你自己使用 vLLM 部署了大模型，可以参考下面的方式。
...
- `global.scrape_interval`：监控间隔时间，推荐为 `3s`。
- `scrape_configs[].metrics_path`： vLLM 服务的 Prometheus 指标接口为 `/metrics`。
- `scrape_configs[].static_configs[].targets`：vLLM 服务的地址和端口，你可能需要根据部署的实际情况进行修改。
...
JSON 内容来自 [https://github.com/vllm-project/vllm/blob/main/examples/production_monitoring/grafana.json](https://github.com/vllm-project/vllm/blob/main/examples/production_monitoring/grafana.json)
